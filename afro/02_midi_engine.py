...
class AgentState:
    def __init__(self, bpm: int, groove_config: dict = None, kg=None):
        self.bpm = bpm
        self.sec_per_beat = 60.0 / bpm
        self.cursor_sec = 0.0
        
        # KG Integration: Resolve genre-aware defaults
        if kg and kg.enabled:
            kg_defaults = kg.resolve_groove_config()
            # Manual blueprint config takes precedence, KG enriches
            full_config = kg_defaults.copy()
            if groove_config:
                full_config.update(groove_config)
            groove_config = full_config
...
def generate_midi(blueprint: dict, outdir: Path, genome: dict = None, kg=None):
    bpm = genome.get("bpm") if genome else blueprint.get("bpm", 124)
    swing = genome.get("swing") if genome else 0.56
    state = AgentState(bpm, groove_config=blueprint.get("groove_config", {}), kg=kg)
    ...
    v_inst = pretty_midi.Instrument(53, name="Vocal Guide")
    ...
    # KG Write-back
    if kg and kg.enabled:
        kg.write("groove_style", state.groove.config.get("shaker_style"), source="GrooveEngine")
        kg.write("note_placement", state.groove.config.get("note_placement"), source="GrooveEngine")
        kg.write("humanization", state.groove.config.get("humanization_intensity"), source="GrooveEngine")

    outdir.mkdir(parents=True, exist_ok=True)
    tracks = {"piano.mid": [p_inst], "bass.mid": [b_inst], "drums.mid": [d_inst, s_inst], "vocal_guide.mid": [v_inst], "full_mix.mid": [p_inst, b_inst, d_inst, s_inst, v_inst]}
...
if __name__ == "__main__":
    ...
    if bp_path.exists(): generate_midi(json.loads(bp_path.read_text()), Path(args.outdir))
